...
class RailEnv(Environment):
    ...
    def __init__(self,
        ...
        self.deadlocks = [0] * number_of_agents
    ...
    def step(self, action_dict_: Dict[int, RailEnvActions]):
        ...
        if self.dones["__all__"]:
            ...
            return self._get_observations(), self.rewards_dict, self.dones, self.deadlocks, info_dict
    ...
    def _step_agent(self, i_agent, action: Optional[RailEnvActions] = None):
        ...
        if np.isclose(agent.speed_data['position_fraction'], 0.0, rtol=1e-03):
            ...
            if action == RailEnvActions.STOP_MOVING and agent.moving:
                ...
                agent.moving = False
                agent_possible_transitions = self.rail.get_transitions(agent.position[0], agent.position[1], agent.direction)
                agent_num_transitions = np.count_nonzero(agent_possible_transitions)
                if (agent_num_transitions > 1) and (self.stop_on_switch_penalty != 0):
                	self.rewards_dict[i_agent] += self.stop_on_switch_penalty
                else:
        ...
        if agent.moving:
            ...
            if np.equal(agent.position, agent.target).all():
                ...
                self._remove_agent_from_scene(agent)
            	  ...
            	  if (np.linalg.norm(np.asarray(agent.position) - np.asarray(agent.target)) <= self.previous_distance[i_agent])and (self.reducing_distance_step != 0):
