...
def main(argv):
    ...
    np.savetxt(fname=path.join('NetsTest' , 'info.txt'), X=[x_dim,y_dim,n_agents,max_num_cities,max_rails_between_cities,max_rails_in_city,tree_depth],delimiter=';')
    ...
    agent.qnetwork_local.load_state_dict(torch.load(path.join('NetsTest' , 'navigator_checkpoint2100_single_standstill.pth')))
    ...
    for trials in range(1, n_trials + 1):
        ...
        print(
                '\rTesting {} Agents on ({},{}).\t Episode {}\t Score: {:.3f}\tDones: {:.2f}%\t Action Probabilities: \t {}'.format(
        ...
        if trials % 50 == 0:
            ...
            np.savetxt(fname=path.join('NetsTest' , 'test_metrics.csv'), X=np.transpose(np.asarray([scores_list,scores,dones_list,dones_list_window])), delimiter=';',newline='\n')
